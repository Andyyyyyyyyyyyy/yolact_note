...
class COCOAnnotationTransform(object):
    """Transforms a COCO annotation into a Tensor of bbox coords and label index  将COCO注释转换成bbox坐标和label索引的tensor
...
class COCODetection(data.Dataset): #继承了pytorch data.Dataset类
    """`MS Coco Detection <http://mscoco.org/dataset/#detections-challenge2016>`_ Dataset.
    Args:
        root (string): Root directory where images are downloaded to. 指向数据集的路径
        set_name (string): Name of the specific set of COCO images.   数据集名称
        transform (callable, optional): A function/transform that augments the   数据增强
                                        raw images`
    ...
    def __init__(self, image_path, info_file, transform=None,
                 ...
                 dataset_name='MS COCO', has_gt=True):  #这里的has_gt，我的理解为has ground truth，就是代表有标注文件
    ...
    def __getitem__(self, index):
        ...
        """
        im, gt, masks, h, w, num_crowds = self.pull_item(index)  #根据索引来选择数据
    ...
    def pull_item(self, index):
        ...
        #区分人群注释。这些注释表示所述类的大量对象，其中没有针对每个对象的注释。在测试和训练中，将这些人群视为中立的。自定义数据集中iscrowd为0
        ...
        #这里的分离是为了兼容COCO2014和2017的注释。2014年，图片的格式是COCO_{train/val}2014_%012d.jpg，而2017年的格式是%012d.jpg。我们的脚本下载的图像为%012d.jpg，因此转换相应。
        file_name = self.coco.loadImgs(img_id)[0]['file_name']
        
        if file_name.startswith('COCO'):
            file_name = file_name.split('_')[-1]  #这里是针对2014数据集的图片

        path = osp.join(self.root, file_name)
        assert osp.exists(path), 'Image path does not exist: {}'.format(path)  #如果图片的路径不存在，则assert（断言）触发异常。
