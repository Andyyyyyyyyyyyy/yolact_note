...
class MultiBoxLoss(nn.Module):
    ...
    Compute Targets:
        1) Produce Confidence Target Indices by matching  ground truth boxes
           ...
           (default threshold: 0.5).  gtbbox和预测bbox之间IOU大于0.5的
    ...
    Objective Loss:
        ...
        Args:
            c: class confidences,  类别置信度
            l: predicted boxes,    预测bbox
            g: ground truth boxes  bbox真值
            N: number of matched default boxes    #bbox匹配个数
        See: https://arxiv.org/pdf/1512.02325.pdf for more details.
    """

    def __init__(self, num_classes, pos_threshold, neg_threshold, negpos_ratio):
        super(MultiBoxLoss, self).__init__()   #调用MultiBoxLoss的构造函数
...
targets, masks, num_crowds都是  [tensor()，tensor()]  这种list<tensor>形式

            targets (list<tensor>): Ground truth boxes and labels for a batch,  
                shape: [batch_size][num_objs,5] (last idx is the label).
targets是bbox和类别的真值，就是标注的值，
targets最后一列代表类别，下边的0应该是car，前边四列是bbox归一化坐标x1,y1,x2,y2

            masks (list<tensor>): Ground truth masks for each object in each image,
                shape: [batch_size][num_objs,im_height,im_width]
mask真值大小和输入图像一致，里边的mask部分是1，其余部分是0

            num_crowds (list<int>): Number of crowd annotations per batch. The crowd
                annotations should be the last num_crowds elements of targets and masks.  这个为0
            
            * Only if mask_type == lincomb
        """

        loc_data  = predictions['loc']  #相当于bbox的系数，边界框回归时使用
        conf_data = predictions['conf'] #类别对应的scores
        mask_data = predictions['mask'] #mask proto 系数
        priors    = predictions['priors']   #anchor机制下的预选框

        if cfg.mask_type == mask_type.lincomb:
            proto_data = predictions['proto']  #mask原型
        ...
        batch_size = loc_data.size(0)  #batch_size
        num_priors = priors.size(0)    #预测头总共输出的目标数
        num_classes = self.num_classes 

        # Match priors (default boxes) and ground truth boxes
        # These tensors will be created with the same device as loc_data
        loc_t = loc_data.new(batch_size, num_priors, 4)     #创建一个新的Tensor，该Tensor的type和device都和原有Tensor一致，且无内容
        ...
        for idx in range(batch_size):
            truths      = targets[idx][:, :-1].data   #每一张图片的边界框真值
            labels[idx] = targets[idx][:, -1].data.long()   ##每一张图片的类别真值
            ...
            gt_box_t[idx, :, :] = truths[idx_t[idx]]  #这个跟match中的matches一样
        ...
        pos = conf_t > 0    #大于0的是正例 TP
        num_pos = pos.sum(dim=1, keepdim=True)   #正例个数
        ...
        if cfg.train_boxes:
            loc_p = loc_data[pos_idx].view(-1, 4)   #预测边界框系数
            loc_t = loc_t[pos_idx].view(-1, 4)      #真值生成的边界框系数
        ...
        if cfg.train_masks:
            if cfg.mask_type == mask_type.direct:
                ...
                if cfg.mask_proto_loss is not None:   # None,
    ...
    def focal_conf_loss(self, conf_data, conf_t):   #conf_data：类别对应的scores，
        """
