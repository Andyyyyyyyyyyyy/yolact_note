...
def postprocess(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
    ...
    Args:
        ...
        - batch_idx: If you have multiple images for this batch, the image's index in the batch.  给出一批图像中选定图像的序号
    ...
    dets = det_output[batch_idx]  #det_output=[{'box':tensor(...),'mask':...,'class':...,'score':...,'proto':...}]里边都是tensor      det_output[0]={'box':...,...}  变为字典
    
    if dets is None:
        return [torch.Tensor()] * 4 # Warning, this is 4 copies of the same thing

    if score_threshold > 0:
        keep = dets['score'] > score_threshold  #过滤

        for k in dets:   #key，就是'box','mask'之类的
            if k != 'proto': 
                dets[k] = dets[k][keep]  #只保留满足条件的元素
        
        if dets['score'].size(0) == 0:   #如果过滤完了，就返回一个空tensor  
    ...
    if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch: 
        ...
        #masks形状为(n,32)   proto_data形状为(138， 138， 32)
        masks = proto_data @ masks.t()  #高维矩阵乘法  (138， 138， n)  用不同目标的对应的系数去乘上proto，得到了对应目标的mask
        masks = cfg.mask_proto_mask_activation(masks)    #sigmoid激活函数

        # Crop masks before upsampling because you know why
        if crop_masks:
            masks = crop(masks, boxes)

        # Permute into the correct output shape [num_dets, proto_h, proto_w]   [n, 138, 138]
        masks = masks.permute(2, 0, 1).contiguous()

        # Scale masks up to the full image   插值之后，变为[n, h, w]
        masks = F.interpolate(masks.unsqueeze(0), (h, w), mode=interpolation_mode, align_corners=False).squeeze(0) 

        # Binarize the masks
        masks.gt_(0.5)    #mask中大于0.5的为1，小于0.5的为0
...
def display_lincomb(proto_data, masks):  #138*138*32  n*32   
    out_masks = torch.matmul(proto_data, masks.t())
    # out_masks = cfg.mask_proto_mask_activation(out_masks)

    for kdx in range(1):   #这里只是仅仅展示了一个目标的prototype，可以修改为masks.shape[0]
        jdx = kdx + 0
        import matplotlib.pyplot as plt
        coeffs = masks[jdx, :].cpu().numpy()
        idx = np.argsort(-np.abs(coeffs))   #将x中的元素从小到大排列，提取其对应的index(索引)，然后输出到y
        # plt.bar(list(range(idx.shape[0])), coeffs[idx])
        # plt.show()
        
        coeffs_sort = coeffs[idx]  #升序排序完的系数列表
        arr_h, arr_w = (4,8)
        proto_h, proto_w, _ = proto_data.size()  #138  138 ——
        arr_img = np.zeros([proto_h*arr_h, proto_w*arr_w])
        arr_run = np.zeros([proto_h*arr_h, proto_w*arr_w])
        test = torch.sum(proto_data, -1).cpu().numpy()  #这里应该变为了138*138*1
