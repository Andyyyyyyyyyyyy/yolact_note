...
class MovingAverage():   #这个是滑动平均窗口???
    ...
    def add(self, elem):
        ...
        if not math.isfinite(elem):   #if not False才会执行if.   math.isfinite判断是否为有限数，是则返回True
    ...
    def get_avg(self):
        """ Returns the average of the elements in the window. #返回窗口中元素的平均值。""" 
...
def init_console():
    ...
    初始化控制台，使其能够在Windows上使用ANSI转义字符。
    """
    if os.name == 'nt':
        from colorama import init  #colorama是一个python专门用来在控制台、命令行输出彩色文字的模块，可以跨平台使用。
        init()
